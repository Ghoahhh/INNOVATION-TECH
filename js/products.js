async function products() {
    await fetch("http://localhost:3000/products").then(r => r.json()).then(data => {
        ...
        filters.addEventListener("input", () => {
            ...
            if(type == "low"); {
                result = result.filter(p => type)
            }

            if(price == "low") {
                result = result.filter(p => result.price <= 200000);
            }

            if(price == "mid") {
                result = result.filter(p => result.price >= 200000 && result.price <= 5000000);
            }

            if(price == "high") {
                result = result.filter(p => result.price > 5000000);
            }
